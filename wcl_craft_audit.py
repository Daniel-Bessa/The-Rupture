...
def _wipe_deaths_ordered(pull_or_wipe: dict, name_fn=None) -> list:
    """Return all deaths sorted chronologically as [(name, time_str), ...]."""
    deaths_src = pull_or_wipe.get("deaths_all") or pull_or_wipe.get("deaths", {})
    fn = name_fn if name_fn is not None else pull_or_wipe.get("_player", lambda pid: f"#{pid}")
    entries = []
    for pid, dlist in deaths_src.items():
        pname = fn(pid)
        for d in dlist:
            entries.append((pname, d.get("time", ""), d.get("fight_pct", 50)))
...
def render_wipe_log_html(pulls: list, boss_name_base: str) -> str:
    ...
    for p in wipes:
        ...
        if cause:
            iss = cause["issue"]
            if "kick" in iss.lower() or "terminate" in iss.lower():
                iss_col = "#d29922"
        ...
        all_deaths = _wipe_deaths_ordered(p)
        def _dcell(idx, _ad=all_deaths):
            if idx < len(_ad):
                nm, t = _ad[idx]
                return (f'<td style="padding:5px 8px;white-space:nowrap;font-size:12px">'
                        f'<span style="color:#e05252">{escape(nm)}</span>'
                        f'<span style="color:#556;font-size:11px"> {escape(t)}</span></td>')
            return '<td style="text-align:center;padding:5px 8px;color:#334">—</td>'
        d_cells = "".join(_dcell(i) for i in range(4))
    ...
    out += ('<div style="overflow-x:auto">'
            ...
            f'<th style="{th.format(align="left")}">1st &#9760;</th>'
            f'<th style="{th.format(align="left")}">2nd &#9760;</th>'
            f'<th style="{th.format(align="left")}">3rd &#9760;</th>'
            f'<th style="{th.format(align="left")}">4th &#9760;</th>'
...
def render_raid_wipe_log_html(wipes: list, boss_name_base: str, actor_lookup: dict) -> str:
    """Render a compact Wipe Log table for the All Wipes pane in per-report raid pages.
    Works with raw fight dicts from _build_boss_html context.
    """
    if not wipes:
        return ""
    ...
    def _name(pid):
        return actor_lookup.get(int(pid), {}).get("name", f"#{pid}")

    rows = ""
    for wi, w in enumerate(wipes, 1):
        cause   = detect_wipe_cause(w, boss_name_base, name_fn=_name)
        dur_str = _dur(w.get("fight_dur_ms", 0) // 1000)
        hp_str  = f"{w.get('boss_pct', 0):.0f}%"
        ...
        if cause and cause.get("mts_label"):
            total_dmg = sum(
                h.get("dmg", 0)
                for pl in w.get("mechanic_timestamps", {}).values()
                for h in pl.get(cause["mts_label"], [])
            )
            if total_dmg:
                dmg_str = _fdmg(total_dmg)
        ...
        all_deaths = _wipe_deaths_ordered(w, name_fn=_name)
        def _dcell(idx, _ad=all_deaths):
            if idx < len(_ad):
                nm, t = _ad[idx]
                return (f'<td style="padding:4px 8px;white-space:nowrap;font-size:12px">'
                        f'<span style="color:#e05252">{escape(nm)}</span>'
                        f'<span style="color:#556;font-size:11px"> {escape(t)}</span></td>')
            return '<td style="text-align:center;padding:4px 8px;color:#334">—</td>'
        d_cells = "".join(_dcell(i) for i in range(4))
        ...
        rows += (
            f'<tr style="border-bottom:1px solid #21262d">'
            f'<td style="padding:4px 8px;white-space:nowrap;color:#aaa">W{wi}</td>'
            f'<td style="padding:4px 8px;text-align:center;color:#667;white-space:nowrap">{dur_str}</td>'
            f'<td style="padding:4px 8px;text-align:center;color:#667">{hp_str}</td>'
            f'<td style="padding:4px 8px;white-space:nowrap">'
            f'<span style="color:{iss_col};font-weight:600">{escape(iss)}</span></td>'
            f'<td style="padding:4px 8px;text-align:center;color:#667">{dmg_str}</td>'
            f'{d_cells}'
            f'<td style="padding:4px 8px;white-space:nowrap">'
            f'<span style="color:{fault_color}">{fault_text}</span></td>'
            f'{seed_cell}'
...
def _build_boss_html(boss_data: dict, actor_lookup: dict, id_prefix: str = "0", wipe_data: dict = None, root: str = "", midnight_rotation: dict = None) -> dict:
    ...
    # ── Shared helpers: defined before the kills loop so they're always bound
    # ── even when boss_data is empty (wipe-only splits from mixed reports).
    # ── The kills loop updates the cell variables in-place; all closures
    # ── capture by reference and see the current value when called.
    mech_defs       = []
    has_interrupts  = False
    has_void_marked = False
    total_cols      = 8
    sorted_pids     = []
    all_pids_set    = set()
    ...
    def pid_sort(pid):
        # Stub used when boss_data is empty (wipe-only path).
        # The kills loop redefines this below with spec-role lookup via `fights`.
        char_name = actor_lookup.get(pid, {}).get("name", "")
        pname, _  = lookup_roster(char_name)
...
"""
    ...
    for boss_name, boss_wipes in (wipe_data or {}).items():
        ...
        mech_defs       = BOSS_MECHANICS.get(boss_name_base, [])
        has_interrupts  = boss_name_base in BOSS_HAS_INTERRUPTS
        has_void_marked = False  # wipe-only: boss was never killed
...
.perf-table th{{background:#111827;color:#7289DA;padding:8px 10px;text-align:center;font-size:11px;letter-spacing:0.5px;text-transform:uppercase;border-bottom:1px solid #2a3a6a;border-right:1px solid #1e2a3a}}
...
.perf-table td{{padding:7px 10px;border-bottom:1px solid #151f2e;border-right:1px solid #1a2233;font-size:12px;text-align:center}}
...
_MECH_SUBTITLE = {
    ...
    "Burning Heart":   "DoT",
    "VL Convergence":  "soak",
    "Void/Lt Flames":  "ground",
    "Void/Lt Edict":   "targeted",
    "Death Drop":      "dodge",
    ...
    "Cosmic Fracture": "burst",
    "Disintegration":  "beam",
    "Heaven's Lance":  "lance",
    "Hvn's Glaives":   "glaives",
    "Dark Rune":       "runes",
    "Light's End":     "ultimate",
    "Tears of L'ura":  "tears",
    "Dark Quasar":     "quasar",
}
...
def write_boss_mythic_html(days_data: list, boss_name: str, output_path: str, guild_name: str = "", midnight_rotation: dict = None) -> None:
    ...
    def _build_totals():
        ...
        if IS_VANGUARD:
            ...
            t += (f'<td style="text-align:center;padding:5px 8px;color:#e3a02e">'
                  f'{_fdmg(t_vdmg) if t_vdmg else "—"}</td>')
        if IS_SALHADAAR:
    ...
    def _build_vanguard_death_table(pulls_subset):
        ...
        """
        # pd[pname][ab] = {"n": int, "pulls": [(pull_num, time_str), ...]}
        pd: dict = {}
        for p in pulls_subset:
            fn, fc_ = p["_player"], p["_color"]
            pull_num = p.get("pull_num", "?")
            for raw_pid, dlist in p["deaths"].items():
                pname = fn(raw_pid)
                color = fc_(raw_pid)
                if pname not in pd:
                    pd[pname] = {"color": color,
                                 **{ab: {"n": 0, "pulls": []} for ab in _VAN_TRACKED},
                                 "other": 0, "total": 0, "multi": 0}
                for death in dlist:
                    ...
                    if hit_abs:
                        for ab in hit_abs:
                            pd[pname][ab]["n"] += 1
                            pd[pname][ab]["pulls"].append((pull_num, death.get("time", "")))
                    else:
                        pd[pname]["other"] += 1
        if not pd:
            return ""
        ...
        out  = ('<div style="margin-top:20px"><div style="color:#e6edf3;font-weight:600;margin-bottom:8px">'
                ...
                'One death can appear in multiple columns. Hover for pull breakdown.</div>'
        ...
        for pname, data in sorted(pd.items(), key=lambda x: -x[1]["total"]):
            ...
            for ab in _VAN_TRACKED:
                ...
                if n > 0:
                    tip_lines = "&#10;".join(f'Pull #{pn} · {_esc(t)}' for pn, t in ab_data["pulls"])
                    out += (f'<td data-val="{n}" style="{col_style};cursor:default" title="{tip_lines}">'
                            f'<span style="color:{col};font-weight:700">{n}</span></td>')
                else:
                    out += f'<td data-val="0" style="{col_style};color:#333">—</td>'
            other = data["other"]
            multi = data["multi"]
            total = data["total"]
            ...
            out += '</tr>'
        # Totals row
        col_totals  = {ab: sum(d[ab]["n"] for d in pd.values()) for ab in _VAN_TRACKED}
...
a{{color:#6e9fcc;text-decoration:none}} a:hover{{text-decoration:underline}}
